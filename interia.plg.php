...
$_pluginInfo=array(
	'name'=>'Interia',
	'version'=>'1.0.8',
	'description'=>"Get the contacts from an Interia.pl account, Plugin developed by Piotr Loposzko",
	'base_version'=>'1.8.0',
	'type'=>'email',
...
class interia extends openinviter_base
{
	...
	public function login($user, $pass)
	{
		...
		$form_action="https://logowanie.interia.pl/poczta/zaloguj";
		$post_elements=array('login'=>$user,
							'pass'=>$pass,
							'domain' => $domain,
							'htmlMail' => 'checked',
							'referer' => 'http://poczta.interia.pl/'
							 );

		$res=$this->post($form_action,$post_elements, true, true, false);
		if ($this->checkResponse("login_post",$res))
			$this->updateDebugBuffer('login_post',"https://logowanie.interia.pl/poczta/zaloguj",'GET');
		else
			{
			$this->updateDebugBuffer('login_post',"https://logowanie.interia.pl/poczta/zaloguj",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}

		$pos = strpos($res, "wyloguj,uid,");
		$this->sid = substr($res, $pos+12, 16);
